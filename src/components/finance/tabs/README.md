...
- `transactions-tab.tsx` -- Full transaction list with filter bar (status/type/category pills), date-grouped TransactionRow items with edit/delete actions, "Cargar mas" pagination, empty state, floating "+" FAB. Exports: `TransactionsTab`.
- `budgets-tab.tsx` -- Budget management with 3 mode UIs: fixed (progress bars), percentage (income reference + allocation pie chart + bars), envelope (Por Asignar hero + envelope cards with Asignar/Quitar). Accepts onAddBudget/onEditBudget/onDeleteBudget callbacks with edit/delete icon buttons per budget row. Exports: `BudgetsTab`.
